...
def chat_with_system_prompt(
    ...
    """
    Gọi API với messages gồm system prompt (persona) + user prompt.
    """
    from openai import OpenAI
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    """
    Đếm số token của một đoạn text bằng thư viện tiktoken, có fallback.
    """
    try:
        import tiktoken
...
def retry_with_backoff(
    ...
    """
    Gọi fn(). Nếu lỗi, thử lại tối đa max_retries lần với backoff cấp số nhân.
    """
    last_exception = None
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        lines.append(
            f"{trunc(r['prompt']):<40} | "
            f"{trunc(r['gpt4o_response']):<40} | "
            f"{trunc(r['mini_response']):<40} | "
